<?php
   ...
   if (!isset($_SESSION['sesion_id'])){
    ...
    $title = array('Nº','Clasificador','Descripción','Cantidad','Monto');
    ...
    $objPHPExcel->getActiveSheet()->getStyle('C6:D6')->applyFromArray($borders_center);    
    $objPHPExcel->setActiveSheetIndex(0)->mergeCellS('C6:D6');
    $objPHPExcel->getActiveSheet()->setCellValue('E6', $title[3]);
    $objPHPExcel->getActiveSheet()->getStyle('E6')->applyFromArray($borders_center);    
    $objPHPExcel->getActiveSheet()->setCellValue('F6', $title[4]);
    $objPHPExcel->getActiveSheet()->getStyle('F6')->applyFromArray($borders_center);

    if(!empty($_Report03)){

        foreach ($_Report03 as $key => $value) {
        $n=$key;
        $i=$key+7;
        $m=$key+1;
        $celdaA='A'.$i;
        ...
        $objPHPExcel->getActiveSheet()->setCellValue($celdaC, $_Report03[$n][1]);
        $objPHPExcel->getActiveSheet()->setCellValue($celdaE, $_Report03[$n][2]);
    ...
    //Dimension
    $objPHPExcel->getActiveSheet()->getColumnDimension("A")->setWidth(5); //70 27
    $objPHPExcel->getActiveSheet()->getColumnDimension("B")->setWidth(13); //70 63
    $objPHPExcel->getActiveSheet()->getColumnDimension("C")->setWidth(60); //210  272
